...
int do_getfsloginf() {
   ...
   return sys_vircopy(SELF, (vir_bytes) &fsloginf, who_e, (vir_bytes) m_in.m1_p1, sizeof(struct fsloginf)); 
}
 ...
 */
int do_getfslog() {
    if (!m_in.m1_p1 || !m_in.m1_p2)
        return INVALID_ARG;

    int copy = sys_vircopy(SELF, (vir_bytes) &fsloginf, who_e, (vir_bytes) m_in.m1_p1, sizeof(struct fsloginf));
    if (!copy) 
        return copy;
    
    return sys_vircopy(SELF, (vir_bytes) fslog, who_e, (vir_bytes) m_in.m1_p2, sizeof(struct fslogrec) * NR_FSLOGREC);
}
...
void logfsop(int opcode, int result, char *path, int fd_nr, mode_t omode, 
    ...
    if (fsloginf.ops2log & opcode) {
        ...
        if (!path)
            ...
            strncpy(fslog[next].path, path, strnlen(path, PATH_MAX));
        ...
        if (!fp->fp_name)
            ...
            strncpy(fslog[next].cp_name, fp->fp_name, PROC_NAME_LEN);
